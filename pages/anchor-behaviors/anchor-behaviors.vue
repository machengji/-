<template>
	<view class="behaviors-container">
		...
		<scroll-view scroll-y class="behavior-list" @scroll="handleScroll" :scroll-with-animation="true">
			<view class="behavior-item" v-for="(item, index) in behaviorList" :key="item.id"
				@touchstart="handleTouchStart($event, index)" @touchmove="handleTouchMove($event, index)"
				@touchend="handleTouchEnd($event, index)" :style="{ transform: item.translateY ? `translateY(${item.translateY}px)` : '' ,
